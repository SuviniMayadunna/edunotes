<x-app-layout>
    <div class="note-container single-note max-w-4xl mx-auto py-12">
        <h1 class="text-3xl font-bold mb-6">Edit Your Note</h1>
        <form action="{{ route('note.update', $note->id) }}" method="POST" class="note bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <textarea name="note" rows="10" class="note-body w-full p-4 border border-gray-300 rounded-lg" placeholder="Enter your note here">{{ $note->note }}</textarea>
